...
```bash
# This week only (no WARP): A fetch → B silver/mart → C Fact Pack → D outlook
docker compose run --rm weekly          # see docs/docker.md
.venv/bin/python scripts/run_weekly_abcd.py --fmt csv
