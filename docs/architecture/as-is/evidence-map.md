...
| R10 | Freshness SLA, completeness SLO, PII/privacy policy, or numeric parsing of actual/forecast. | unknown / explicitly deferred | `docs/architecture.md` “What we deliberately skip”; no tickets or SLO files |
| R10a | A late-arriving `actual` must never be overwritten by an empty value, and revisions must be auditable. | fact (implemented) | `scripts/transform/merge_silver.py` `upsert_silver` coalesces field by field; delta log in `data/silver/calendar_event_changes/`; `tests/test_merge_silver.py` |
| R10b | Row identity is the absolute release instant, not the published clock — the two feeds publish in different timezones. | fact (measured) | Across weekly (US Eastern) and monthly (Asia/Novosibirsk) silver, a key with `time_raw` matched 0 rows; `(event_datetime_utc, currency, event)` matched 91 of 92. `merge_silver.event_uid` |
| R10c | `fact_id` must be stable across rebuilds, because analyst prompts compare actual vs forecast on the same `fact_id`. | fact (implemented) | `docs/analyst/semantics/macro_textbook.md` requires it; `to_kimball` now derives `fact_id` from `event_uid` instead of `range(1, n+1)` |
| R10d | Bronze raw retention: ICT snapshots are gzipped and capped per series (`ICT_BRONZE_KEEP`, default 4). | fact (implemented) | `scripts/extract/ict_sources.py` `_write_bronze_snapshot`; previously ~1MB per weekly run with no cleanup |
| R10e | A monthly partition missing an impact layer must warn, not pass silently. | fact (implemented) | `scripts/transform/validate_coverage.py`, called after step B; nine months sat at `red+orange+yellow` with the gray layer never fetched |
